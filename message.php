	...
	<div class="container">
		...
		<div class="row d-flex justify-content-center">
			<form action="messageExport.php"> 
				<fieldset>
					<button class="genric-btn primary-border circle" type="submit" name="esportaUser" value="Esporta in Excel tutti i messaggi" />Esporta in Excel</button>
				</fieldset>
			</form>
		</div>
				...
				<table>
					...
					foreach ($messageUsers as $message) { ?>
						<tr>
							...
							<td><?php echo $message['UserID']; ?></td>
							<td><?php echo $message['Text']; ?></td>
							<td>
							<form method="post" action="sendSingle.php" method="POST">
								<textarea class="common-textarea form-control" id="send<?php echo $message['ID'] ?>" name="testo" placeholder="Messaggio" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Messeggio'" required="1"></textarea>	
								...
								<br>
								<button class="genric-btn primary-border circle" type="submit" id="invia" name="invia" value="Invia" />Invia</button>
							</form>
							</td>
							<td>
